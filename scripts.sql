...
CREATE TABLE company_users(id SERIAL PRIMARY KEY, user_id int REFERENCES people (id), company_id int REFERENCES companies (id));
CREATE TABLE transactions(id SERIAL PRIMARY KEY, user_id int REFERENCES people(id), company_id int REFERENCES companies(id), amount numeric(32, 2), type varchar(10));
